...
| `4` | питание | GND | Земля |
| `5` | сигнал | Видна дорожка к стороне MCU | Неизвестная цифровая или служебная линия |
| `6` | сигнал | Видна дорожка к стороне MCU | Неизвестная цифровая или служебная линия |
| `7` | сигнал | Видна дорожка к стороне MCU | Неизвестная цифровая или служебная линия |
| `8` | питание | 3.3 V | Питание Wi-Fi-модуля |

Посадочное место размером 2x4 и диагональные `3.3 V`/GND совместимы с идеей ESP-01, но это только гипотеза по форм-фактору. Она не доказывает ни тип модуля, ни наличие UART, ни порядок `TX`, `RX`, `EN`, `RST`, `GPIO0` или `GPIO2`.
...
Это даёт питание, землю и все три подтверждённо разведённые сигнальные площадки верхнего ряда. `SIG-A/B/C` - только нейтральные имена площадок: они не являются предполагаемыми `TX`, `RX` или `EN`. Контакт `1` стоит оставить доступным тестовой площадкой: он тоже имеет видимую дорожку и может оказаться четвёртым сервисным сигналом.
...
3. После прозвонки сопоставить найденные GPIO с физической таблицей MCU и установить, чем являются линии на этой конкретной плате. Только если две из них окажутся свободной UART-парой, можно назвать их `TX` и `RX`.
