...
![Размеченная плата SC-60-V2-L-V1.0](../assets/laser-driver-sc-60-v2-l-annotated.png)

Разметка фиксирует уже проверенные шины и точки прозвонки. В частности, по одной фотографии нельзя доказать, что `PE3` управляет low-side каскадом: его маршрут до линейной гребёнки был подтверждён прозвонкой на стороне Beta M, но функция этой линии внутри SC-60 остаётся неподтверждённой.
