...
- Контроллер: **Beta M V1.1**, маркировка платы `20201207`; MCU `STM32F407VGT6` в LQFP100.
...
- `assets/` - фото идентификации платы без GPS/EXIF-метаданных.
- `firmware/` - текущая экспериментальная сборка и её SHA-256.

В репозитории намеренно нет экспериментальных DWIN-пакетов и диагностических BIN. Вложенная прошивка отмечена как экспериментальная: обходы термозащиты не являются переносимым решением для другого принтера.
